...
describe("LengthValidator", () => {
  ...
  describe("validate method", () => {
    it("should return no error when value length is within the specified range", () => {
      const value = "abcd";
      const schema = {
        min: { value: 2, errorMessage: "Min length error" },
        max: { value: 5, errorMessage: "Max length error" },
      };
    ...
    it("should return an error when both min and max constraints are violated", () => {
      const value = "abc";
      const schema = {
        min: { value: 4, errorMessage: "Min length error" },
        max: { value: 2, errorMessage: "Max length error" },
      };
      const result = lengthValidator.validate(value, schema);
