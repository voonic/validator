...
describe("DependsOnValidator", () => {
  describe("validate", () => {
    it("should validate numeric input with min constraint", () => {
      ...
      const result = validator.validate(5, schema, inputType, dependsValue);
      const result2 = validator.validate(7, schema, inputType, dependsValue);
    ...
    it("should validate numeric input with max constraint", () => {
      ...
      const result = validator.validate(32, schema, inputType, dependsValue);
      const result2 = validator.validate(31, schema, inputType, dependsValue);
      expect(result.error).toBeTruthy();
      expect(result.errorMessage).toBe("Invalid value");
      expect(result2.error).toBeFalsy();
    });
    ...
    it("should return valid for numeric input within range", () => {
      ...
      const inputType = "numeric";
      const dependsValue = 10;
      const result = validator.validate(8, schema, inputType, dependsValue);
      const result2 = validator.validate(10, schema, inputType, dependsValue);
    ...
    it("should throw an error for unknown input combination for numeric", () => {
      ...
      expect(() =>
        // @ts-ignore
        validator.validate(9, schema, inputType, dependsValue)
      ).toThrow(
